...
import { Search, FileText, Eye, TrendingUp, Users, BookOpen, Filter, X, Download } from "lucide-react";
...
export default function DesempenhoAdmin() {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 md:p-8 max-w-[1600px] mx-auto w-full">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">Monitoramento pedagógico e análise de indicadores</p>
          </div>
          ...
          <section className="bg-white dark:bg-slate-900 rounded-2xl md:rounded-3xl p-4 md:p-6 shadow-sm border border-slate-200 dark:border-slate-800 mb-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              <div className="relative">
                ...
                <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar aluno..." className="w-full pl-10 pr-4 py-3 rounded-xl border dark:bg-slate-800 dark:text-white dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <select value={turma} onChange={(e) => setTurma(e.target.value)} className="w-full px-4 py-3 rounded-xl border dark:bg-slate-800 dark:text-white dark:border-slate-700">
                <option value="">Todas turmas</option>
                <option>1º Ano C</option>
                <option>2º Ano A</option>
                <option>3º Ano B</option>
              </select>
              <select value={situacao} onChange={(e) => setSituacao(e.target.value)} className="w-full px-4 py-3 rounded-xl border dark:bg-slate-800 dark:text-white dark:border-slate-700">
              ...
              <div className="flex gap-2">
                <button className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-4 py-3 font-semibold">
                  <Filter size={18} /> Aplicar
                </button>
                {(busca || turma || situacao) && (
                  <button onClick={limparFiltros} className="p-3 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-xl hover:bg-red-50 hover:text-red-500">
                    <X size={18} />
                  </button>
                )}
              </div>
          ...
          <section className="bg-white dark:bg-slate-900 rounded-2xl md:rounded-3xl p-4 md:p-6 shadow-sm border border-slate-200 dark:border-slate-800">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <h2 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">Desempenho por Aluno</h2>
              <div className="flex gap-2 w-full sm:w-auto">
                <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold">
                  <FileText size={16} /> Gerar PDF
                </button>
                <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-slate-800 text-white px-5 py-2.5 rounded-xl text-sm font-semibold">
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="border-b dark:border-slate-700 text-slate-500 text-sm">
                    <th className="text-left py-4 px-3">Aluno</th>
                    <th className="text-left py-4 px-3">Turma</th>
                    <th className="text-left py-4 px-3">Média</th>
                    <th className="text-left py-4 px-3">Frequência</th>
                    <th className="text-left py-4 px-3">Situação</th>
                    <th className="text-right py-4 px-3">Ações</th>
                  </tr>
                </thead>
                  ...
                  {filtrados.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="py-4 px-3">
                        <span className={`font-bold ${item.media >= 7 ? 'text-green-600' : item.media >= 5 ? 'text-amber-500' : 'text-red-500'}`}>{item.media.toFixed(1)}</span>
                      </td>
                      ...
                      <td className="py-4 px-3 text-right">
                        <button className="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-lg shadow-sm"><Eye size={14} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {filtrados.length === 0 && <div className="text-center py-12 text-slate-400">Nenhum aluno encontrado.</div>}
          </section>
        </main>
        <Footer />
      </div>
...
// Componentes auxiliares (StatCard, ChartCard, CustomTooltip) – iguais ao seu original
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`text-xs font-bold ${trend.startsWith('+') ? 'text-green-500' : trend.includes('%') ? 'text-blue-500' : 'text-slate-400'}`}>{trend}</span>
      </div>
